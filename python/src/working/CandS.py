...
def calculate_homography(msgData):
    dest_points = np.array(
        [(0.1, 0.9), (0.5, 0.9), (0.9, 0.9), (0.1, 0.5), (0.5, 0.5), (0.9, 0.5), (0.1, 0.1), (0.5, 0.1), (0.9, 0.1)]);
...
def getGazePointWithConf(sub, conf):
    ...
    confidence = getConf(msg)

    xy_coord = getRawGazePoint(msg)
...
def send_uncalibrated(sub, sock, ip, port):
    while (True):
        ...
        xy_coord = getGazePointWithConf(sub, 0.65)

        if (xy_coord[0] != -1 and xy_coord[1] != -1):
            print("uncalibrated =" + str(xy_coord[0]) + "," + str(xy_coord[1]))
...
def send_calibrated(sub, sock, ip, port, homography_matrix):
    while (True):
        ...
        if (xy_coord[0] != -1 and xy_coord[1] != -1):
            ...
            print("calibrated =" + str(calibrated_point[0]) + "," + str(calibrated_point[1]))
